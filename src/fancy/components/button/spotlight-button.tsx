...
export const SpotlightButton: React.FC<SpotlightButtonProps> = ({ children }) => {
  return (
    <div style={{ transform: "none" }}>
      <button className="group relative inline-block cursor-pointer rounded-xl bg-zinc-900 p-px font-semibold leading-6 text-white no-underline shadow-2xl shadow-zinc-900">
        ...
        <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-[#FF4D4D]/0 via-[#F9CB28]/90 to-[#FF8F00]/0 transition-opacity duration-500 group-hover:opacity-40"></span>
      </button>
    </div>
  );
}
